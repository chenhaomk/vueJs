define(['axio', 'vue', 'croppie'], function (axio, Vue, croppie) {
    ...
    // axio.defaults.baseURL = 'https://api.yingougou.com/v1.1';//生成地址   
    var ygg = {};
    ...
    ygg.indexPrompt = function (t) {
        ...
        if (!dom) {
            var prompt = document.createElement("section"),
                ...
                span = document.createElement("span");
            div.setAttribute("class", "index_div");
            div.innerHTML = '<img class="index_rocket" src="./assets/images/index/ic_pic_xxh.png"><span class="index_span" >程序员火速开发中</br>敬请期待!</span><div class="btn" >知道了</div>'
            prompt.setAttribute("id", "prompt");
            prompt.setAttribute("class", "prompt");
            shadow_b.setAttribute("class", "shadow_b");
    ...
    // //专题列表
    // ygg.template.specialLsit = Vue.extend({
    //     props: {
    //         a: {
    //             type: Object
    //         },
    ...
    //专题商铺列表
    ygg.template.specialDetailshopList = Vue.extend({
        props: {
            a: {
                type: Object
            },
        ...
        template: '<li><a :href="url+a.business_id">\
                          <img :src="a.business_img">\
                          <section class="text">\
                              <p class="title">{{a.business_name}}</p>\
                              <score :score="a.business_star*10"></score>\
                              <a class="address"><span class="text-sl">{{a.business_area}}</span><span class="text-r" >{{a.business_distance > 0?(a.business_distance/1000).toFixed(1):0}}km</span></a>\
                          </section>\
                      </a></li>',
        components: {
            score: ygg.template.star
